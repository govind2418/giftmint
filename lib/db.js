  ...
  await conn.query(
    `INSERT INTO orders
      (order_id, user_email, customer_name, customer_email, order_date, created_at, address, items,
       ...
       partner_name, partner_order_ref, is_gift, recipient_name, recipient_phone, recipient_address, gift_message)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [
      ...
      order.recipientAddress || null, order.giftMessage || null
    ]
  );
}
...
async function setOrderGiftRecipient(orderId, gift) {
  await pool.query(
    'UPDATE orders SET is_gift = 1, recipient_name = ?, recipient_phone = ?, recipient_address = ?, gift_message = ? WHERE order_id = ?',
    [gift.name, gift.phone, gift.address, gift.message || null, orderId]
  );
}
