...
async function init() {
  ...
  // Migration: "Send to a Friend" - the buyer pays, but the codes should be
  // delivered to someone else. is_gift flags the order so the admin
  // dashboard and invoice both surface the recipient's contact info instead
  // of (or alongside) the buyer's own, so fulfillment reaches the right
  // person.
  await ensureColumn('orders', 'is_gift', 'TINYINT(1) NOT NULL DEFAULT 0');
  await ensureColumn('orders', 'recipient_name', 'VARCHAR(255) NULL');
  await ensureColumn('orders', 'recipient_phone', 'VARCHAR(20) NULL');
  await ensureColumn('orders', 'recipient_address', 'TEXT NULL');
  await ensureColumn('orders', 'recipient_email', 'VARCHAR(255) NULL');
  await ensureColumn('orders', 'gift_message', 'TEXT NULL');
  // Same recipient details, carried on a still-pending direct-UPI order so
  // they survive from create -> admin approval -> the real order row above.
  await ensureColumn('pending_upi_orders', 'gift', 'JSON NULL');

  // Same idempotency guarantee as above, at the DB level, for the Razorpay
  // webhook's payment-redelivery case - closes the tiny race window where
  // two near-simultaneous retries both pass the "no existing order yet"
  // check before either has committed.
  await ensureUniqueIndex('orders', 'uniq_razorpay_payment_id', ['razorpay_payment_id']);

  // Migration: phone-number + OTP is now the customer auth method (no more
  // password login). email stays the primary key (orders/sessions already
  // reference it), phone is just an additional unique lookup column.
  await ensureColumn('users', 'phone', 'VARCHAR(15) NULL');
  await ensureUniqueIndex('users', 'uniq_user_phone', ['phone']);
...
async function createUser({ email, name, phone, passwordSalt, passwordHash, auto = false }) {
  await pool.query(
    'INSERT INTO users (email, name, phone, password_salt, password_hash, is_auto, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)',
    [email, name, phone || null, passwordSalt, passwordHash, auto ? 1 : 0, Date.now()]
  );
}
  ...
  await conn.query(
    `INSERT INTO orders
      (order_id, user_email, customer_name, customer_email, order_date, created_at, address, items,
       ...
       partner_name, partner_order_ref, is_gift, recipient_name, recipient_phone, recipient_address, recipient_email, gift_message)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [
      ...
      order.isGift ? 1 : 0, order.recipientName || null, order.recipientPhone || null,
      order.recipientAddress || null, order.recipientEmail || null, order.giftMessage || null
    ]
  );
}

// Tags an already-created order as a gift and records where it should
// actually be delivered - called from the post-checkout "Send to a Friend"
// flow, separate from order creation itself.
async function setOrderGiftRecipient(orderId, gift) {
  await pool.query(
    'UPDATE orders SET is_gift = 1, recipient_name = ?, recipient_phone = ?, recipient_address = ?, recipient_email = ?, gift_message = ? WHERE order_id = ?',
    [gift.name, gift.phone, gift.address, gift.email || null, gift.message || null, orderId]
  );
}
...
// Razorpay can redeliver the same payment.captured webhook more than once
// (at-least-once delivery) - this lets the webhook handler recognize a
// payment it already turned into an order instead of minting a second one.
async function getOrderByRazorpayPaymentId(razorpayPaymentId) {
  const [rows] = await pool.query('SELECT * FROM orders WHERE razorpay_payment_id = ?', [razorpayPaymentId]);
  return rowToOrder(rows[0]);
}
...
function rowToPendingUpiOrder(row) {
  if (!row) return null;
  const items = typeof row.items === 'string' ? JSON.parse(row.items) : row.items;
  const gift = row.gift ? (typeof row.gift === 'string' ? JSON.parse(row.gift) : row.gift) : null;
  return {
...
async function createPendingUpiOrder(p) {
  await pool.query(
    `INSERT INTO pending_upi_orders (id, user_email, items, subtotal, platform_fee, grand_total, status, created_at, gift)
     VALUES (?, ?, ?, ?, ?, ?, 'pending', ?, ?)`,
    [p.id, p.userEmail, JSON.stringify(p.items), p.subtotal, p.platformFee, p.grandTotal, Date.now(), p.gift ? JSON.stringify(p.gift) : null]
  );
}
...
module.exports = {
  init,
  getUserByEmail, getUserByPhone, getUserByToken, createUser, updateUserPassword, updateUserAddress, updateUserProfile, countUsers, getUserTypeCounts, getAllUsersWithStats,
  addSession, removeSession,
  addAdminSession, removeAdminSession, isAdminSessionValid,
  createOrder, getOrdersForUser, getOrderById, getOrderByPartnerRef, getOrderByRazorpayPaymentId, getAllOrders, getOrderStats, refreshStatuses,
  setOrderGiftRecipient,
