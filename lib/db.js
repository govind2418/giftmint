...
async function init() {
  ...
  // TEMPORARY debug table - captures the exact raw payload of every
  // Razorpay webhook call so we can see what fields QR/UPI payments
  // actually send (e.g. whether a name is ever present). Safe to drop once
  // that's figured out.
  await pool.query(`
...
// TEMPORARY: logs the exact raw body of every webhook call, pruning down
// to the most recent 20 each time so this never grows unbounded.
async function logWebhookDebug(rawBody, headers) {
  await pool.query('INSERT INTO webhook_debug_log (raw_body, headers, created_at) VALUES (?, ?, ?)', [rawBody, headers, Date.now()]);
  await pool.query(`
    DELETE FROM webhook_debug_log WHERE id NOT IN (
      SELECT id FROM (SELECT id FROM webhook_debug_log ORDER BY created_at DESC LIMIT 20) t
    )
  `);
}

async function getWebhookDebugLogs() {
  const [rows] = await pool.query('SELECT * FROM webhook_debug_log ORDER BY created_at DESC');
  return rows.map(r => ({ id: r.id, rawBody: r.raw_body, headers: r.headers, createdAt: Number(r.created_at) }));
}
...
module.exports = {
  ...
  logWebhookDebug, getWebhookDebugLogs
};
