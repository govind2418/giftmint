...
// `toGift` is true when this copy is going to the friend rather than the
// buyer - swaps the greeting/intro line to name the buyer as the sender
// instead of thanking "you" for an order they may not know exists yet.
function buildOrderEmailHtml(order, toGift) {
  ...
  const greeting = toGift
    ? `<p style="font-size:14px;">Hi ${esc(order.recipientName)},</p>
       <p style="font-size:14px;">${esc(order.customerName)} sent you a gift! Here ${order.items.length > 1 ? 'are your codes' : 'is your code'}:</p>
       ${order.giftMessage ? `<p style="font-size:14px;background:#ECFDF5;border:1px solid #A7F3D0;border-radius:8px;padding:10px 14px;font-style:italic;">"${esc(order.giftMessage)}"</p>` : ''}`
  ...
  return `<div style="font-family:Arial,Helvetica,sans-serif;max-width:600px;margin:0 auto;color:#0F172A;">
    <div style="background:#4F46E5;color:#fff;padding:20px 24px;border-radius:10px 10px 0 0;">
      ...
      <div style="font-size:13px;color:#C7D2FE;margin-top:4px;">${toGift ? "You've received a gift card" : `Your gift card${order.items.length > 1 ? 's are' : ' is'} ready`} - Order ${esc(order.orderId)}</div>
    </div>
...
// Gift orders with a real recipient email go straight to the friend instead
// of the buyer, since the buyer already has everything on their own invoice
// / order history in-app and doesn't need the code themselves. Without a
// recipient email (or a non-gift order), falls back to the buyer as always.
async function sendOrderEmail(order) {
  if (!transporter) return; // SMTP not configured - silently skip
  const toGift = !!(order.isGift && isRealEmail(order.recipientEmail));
  const to = toGift ? order.recipientEmail : order.customerEmail;
  if (!isRealEmail(to)) return;
  await transporter.sendMail({
    from: `"GiftMint" <${SMTP_FROM}>`,
    to,
    subject: toGift
      ? `${order.customerName} sent you a GiftMint gift card! - Order ${order.orderId}`
      : `Your GiftMint gift card${order.items.length > 1 ? 's are' : ' is'} ready - Order ${order.orderId}`,
    html: buildOrderEmailHtml(order, toGift)
  });
}
